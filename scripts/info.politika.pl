...
# http://www.b92.net/info/vesti/index.php?yyyy=2014&mm=05&dd=10&nav_category=16
# where nav_category=16 represents that category=hronika
...
if (open(LINKS, '<','../corpus/info.politika/links'))
{
	while (my $line = <LINKS>)
	{
...
for (my $i = 0; $i <= $j; $i++ )
{
	...
	my $url = "http://www.b92.net/info/vesti/index.php?yyyy=".$year."&mm=".$month."&dd=".$day."&nav_category=11";
	$mech->get( $url );
	...
	# print $month." ".$day."\n";
	my @current_page_links = $mech->find_all_links( url_regex => qr/^\/info\/vesti\/index.php\?yyyy=$year\&mm=$month\&dd=$day\&nav_category=11/);
	for my $link ( @current_page_links ) {
		...
		if (not (grep $_ eq $current_link, @all_category_links) or not @all_category_links)
		{
			...
			open(LINKS, '+>>', '../corpus/info.politika/links') or die "Cannot open links file!\n";
...
sub download_page
{
	...
	foreach my $span_iter (@span) {
		...
		if ($span_attr_class and $span_attr_class eq "article-info1")
		{
			...
			if ($span_iter->innerText() =~ /(.*)\|\s+(\w+)\s+(\d+\.\d+\.\d+)|(.*)/)
			{
				$date = "$2"." "."$3";
			}
		}
	...
	open(FH, ">../corpus/info.politika/$file_name") or die "Cannot create file!\n";
	
	my $header_source = "> Izvor: B92\n";
	my $header_date = "> Datum: ";
	my $header_group = "> Rubrika: info.politika\n";
