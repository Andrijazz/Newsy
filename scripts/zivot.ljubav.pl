...
if (open(LINKS, '<','../corpus/život.ljubav/links'))
{
	while (my $line = <LINKS>)
	{
...
my $starting_url = "http://www.b92.net/zivot/ljubav.php";
$mech->get( $starting_url );
my $i = 1;

while ($i <= 33)
{
	my @current_page_links = $mech->find_all_links( url_regex => qr/^\/zivot\/ljubav\.php\?yyyy/ );
	for my $link ( @current_page_links ) {
		...
		if (not (grep $_ eq $current_link, @all_category_links) or not @all_category_links)
		{
			...
			open(LINKS, '+>>', '../corpus/život.ljubav/links') or die "Cannot open links file!\n";
...
sub download_page
{
	...
	open(FH, ">../corpus/život.ljubav/$file_name") or die "Cannot create file!\n";
	
	my $header_source = "> Izvor: B92\n";
	my $header_date = "> Datum: ";
	my $header_group = "> Rubrika: život.ljubav\n";
