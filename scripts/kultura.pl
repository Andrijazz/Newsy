...
if (open(LINKS, '<','../corpus/kultura/links'))
{
	while (my $line = <LINKS>)
	{
...
for (my $i = 0; $i <= $j; $i++ )
{
	...
	# http://www.b92.net/kultura/vesti.php?nav_category=1087&yyyy=2014&mm=05&dd=17&nav_id=848971
	my $url = "http://www.b92.net/kultura/vesti.php?yyyy=".$year."&mm=".$month."&dd=".$day;
	...
	my @current_page_links = $mech->find_all_links( url_regex => qr/^\/kultura\/vesti.php\?nav_category=(268|270|271|272|321|1087)\&yyyy=$year\&mm=$month\&dd=$day/);
	for my $link ( @current_page_links ) {
		...
		if (not (grep $_ eq $current_link, @all_category_links) or not @all_category_links)
		{
			...
			open(LINKS, '+>>', '../corpus/kultura/links') or die "Cannot open links file!\n";
...
sub download_page
{
	...
	open(FH, ">../corpus/kultura/$file_name") or die "Cannot create file!\n";
	
	my $header_source = "> Izvor: B92\n";
	my $header_date = "> Datum: ";
	my $header_group = "> Rubrika: kultura\n";
