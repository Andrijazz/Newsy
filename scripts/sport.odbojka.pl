...
if (open(LINKS, '<','../corpus/sport.odbojka/links'))
{
	while (my $line = <LINKS>)
	{
...
for (my $i = 0; $i <= $j; $i++ )
{
	...
	my $url = "http://www.b92.net/sport/odbojka/vesti.php?yyyy=".$year."&mm=".$month."&dd=".$day;
	...
	my @current_page_links = $mech->find_all_links( url_regex => qr/^\/sport\/odbojka\/vesti.php\?yyyy=$year\&mm=$month\&dd=$day/);
	for my $link ( @current_page_links ) {
		...
		if (not (grep $_ eq $current_link, @all_category_links) or not @all_category_links)
		{
			...
			open(LINKS, '+>>', '../corpus/sport.odbojka/links') or die "Cannot open links file!\n";
...
sub download_page
{
	...
	open(FH, ">../corpus/sport.odbojka/$file_name") or die "Cannot create file!\n";
	
	my $header_source = "> Izvor: B92\n";
	my $header_date = "> Datum: ";
	my $header_group = "> Rubrika: sport.odbojka\n";
